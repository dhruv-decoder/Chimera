...
**Combined attack chains (beta).** Real campaigns rarely use one technique, so I probed a two-stage chain (isolated from the shipped pipeline): a synthetic-identity bust-out ages a set of accounts that then run a low-observability, authorised-push-style cash-out. Measured on the stage-2 fraud at a matched 1% alert budget, chaining collapses the *supervised* classifier from 38% to 8% recall, the novelty channel holds it at 62%, and retraining restores supervised recall to 94%. It is the same lesson as the agent-hijack result on a multi-stage threat: the two-channel design catches what the classifier alone misses, and the loop closes the gap. Marked beta because the chain is scripted rather than driven by the evolutionary search.
